...
export const KitchenCarousel = ({ cooked }: { cooked: Cooked[] }) => {
  ...
  const [missingIngredients, setMissingIngredients] = useState<NeededIngredient[]>([])
      ...
      for (const inv of data) {
        ...
        if (!inInv) {
          ...
          missing.push({
            ...
            recipeId: recipe.id,
            inventoryQuantity: inv.inventoryQuantity - inInv.currentStock,
  ...
  return (
    <div>
      <AlertDialog open={alertOpen} defaultOpen={false}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Not Enough Ingredients</AlertDialogTitle>
            <AlertDialogDescription>
                {missingIngredients.map((ingr, i) => (
                  <span key={i}>{ingr.inventoryName} - {ingr.inventoryQuantity}</span>
                ))}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
