...
export default function KitchenPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [r1, r2] = await Promise.all([fetch("/api/recipes"), fetch("/api/mealtypes")])
  ...
  const itemCard = (name: string, cost: number, image: string | null) => (
    <div className="group relative bg-white rounded-xl overflow-hidden shadow-md hover:shadow-xl transition-all duration-300 hover:scale-[1.05] border border-stone-100">
      <div className="relative w-full h-40 bg-gradient-to-br from-stone-50 to-stone-100 overflow-hidden">
        <img
          src={image || "/placeholder.svg?height=160&width=160&query=food"}
          alt={name}
      ...
      <div className="p-3 bg-white">
        <p className="text-sm font-bold text-stone-900 line-clamp-2 leading-tight">{name}</p>
      </div>
    </div>
  )
  ...
  return (
    <div className="min-h-screen w-full bg-white">
      <div className="bg-maroon-gradient text-white px-8 py-12 shadow-lg">
        <div className="max-w-7xl mx-auto">
          <h1 className="text-5xl md:text-6xl font-black tracking-tight mb-2">Our Menu</h1>
          <p className="text-lg text-maroon-50 font-medium">Crafted with quality & tradition</p>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-8 py-12">
        {/* Meal Plans Section */}
        <div className="mb-16">
          <h2 className="text-3xl font-black text-maroon mb-8 tracking-tight">Meal Plans</h2>
              ...
              .map((m, i) => (
                ...
                >
                  <div className="bg-gradient-to-br from-[#ce123d] to-[#a50a2f] h-24 flex items-center justify-center relative overflow-hidden">
                    ...
                    <h3 className="text-2xl font-black text-white relative z-10 text-center px-4">{m.typeName}</h3>
                  </div>
                  <div className="p-4">
                    <div className="text-center mb-4">
                      <p className="text-4xl font-black text-[#ce123d]">${price(m.price)}</p>
                      <p className="text-xs text-stone-500 font-medium mt-1 uppercase tracking-wide">per meal</p>
                    </div>
                    <div className="space-y-2 border-t border-stone-100 pt-4">
                      {m.entrees !== undefined && m.entrees != 0 && (
                        <div className="flex items-center justify-between">
                          <span className="text-sm text-stone-700 font-semibold">Entrees</span>
                      ...
                      {m.sides !== undefined && m.sides != 0 && (
                        <div className="flex items-center justify-between">
                          <span className="text-sm text-stone-700 font-semibold">Sides</span>
                      ...
                      {m.drinks !== undefined && m.drinks != 0 && (
                        <div className="flex items-center justify-between">
                          <span className="text-sm text-stone-700 font-semibold">Drinks</span>
        ...
        <div className="mb-16">
          <div className="flex items-center gap-3 mb-8">
            ...
            <h2 className="text-3xl font-black text-maroon tracking-tight">Entrees</h2>
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 lg:grid-cols-6 gap-4">
            {recipes
              .filter((r) => r.type === "Entree")
              .map((r, i) => (
                <div key={i}>{itemCard(r.name, r.pricePerServing, r.image)}</div>
              ))}
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
          ...
          <div>
            <div className="flex items-center gap-3 mb-8">
              <div className="w-1 h-10 bg-gradient-to-b from-[#ce123d] to-[#500000] rounded-full"></div>
              <h2 className="text-3xl font-black text-maroon tracking-tight">Sides</h2>
            </div>
          ...
          <div>
            <div className="flex items-center gap-3 mb-8">
              <div className="w-1 h-10 bg-gradient-to-b from-[#ce123d] to-[#500000] rounded-full"></div>
              <h2 className="text-3xl font-black text-maroon tracking-tight">Drinks</h2>
            </div>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              {recipes
